...
**深度研究助手**（综合案例 02，从零复刻版）。输入一个研究主题，工具自动完成检索、阅读、迭代、综合，产出带来源引用的研究报告。

- **`README.md` 是唯一的规格来源**，所有产品要求以此为准；`任务说明书.md` 是六要素拆解与子任务验收标准。
- **本期只做后端**（`backend/`），前端后续补充。
- 当前处于**从零实现中**：按 `任务说明书.md` 的子任务 T1~T11 顺序推进，每个模块实现完必须能单跑自检。
...
README 定义的核心流程是一个 agentic loop，不是单次问答：
...
- **过程记录要落盘**：检索关键词、已读页面、迭代轮数都要记录下来。

## 搜索工具：Bocha 网页搜索 API

- 端点：`POST https://api.bocha.cn/v1/web-search`
- 鉴权：请求头 `Authorization: Bearer <API key>`（key 在 `.env` 的 `BOCHA_API_KEY`）
- 请求体参数：`query`（必填）、`summary`（是否返回摘要）、`count`（返回条数，默认 10）
...
├── config.py       # 从项目根 .env 读配置（DeepSeek / Bocha key、RESEARCH_MAX_ROUNDS 等）
...
- **搜索工具**：Bocha web-search（`tools.web_search`，普通 async 函数，返回解析后的 `list[dict]`：title/url/snippet/site_name/date）；不做 `fetch_page`（总结基于搜索结果摘要）。
...
- **代码注释、docstring、日志文案统一用中文**，与现有风格保持一致。

## 参考代码

- 同级 `综合案例-02/`：**本项目的完整参考实现（对照答案）**——实现完成后逐项对比，但实现过程中不要照抄。
- 父目录 `09-参考代码/`：`openai-agents.py`（agent 范式）、`jinja2_demo.py`、`fastapi_demo.py`。
